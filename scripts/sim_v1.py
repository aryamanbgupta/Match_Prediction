...
class MatchState():
    ...
    # Batsman scores: batsman_idx -> (runs, balls)
    ...
    def update(self, outcome:Outcome, runs:int=0):
        ...
        if outcome not in [Outcome.WIDE]:
...
class T20Rules:
    ...
    def __init__(self, bowler_selector: Optional[BowlerSelector] = None):
        self.bowler_selector = bowler_selector or RandomBowlerSelector()
    ...
    def process_ball(self, state: MatchState, outcome: Outcome) -> int:
        ...
        # Direct run outcomes
        if outcome in [Outcome.ONE, Outcome.TWO, 
                      Outcome.FOUR, Outcome.SIX]:
            runs = outcome.value
        ...
        elif outcome == Outcome.BYE:
            runs = 1  # Simplified
        elif outcome == Outcome.LEG_BYE:
            runs = 1  # Simplified
    ...
    def handle_over_end(self, state: MatchState):
        """Handle end of over transitions"""
        # Select new bowler
        state.bowler_idx = self.select_next_bowler(state)

    def handle_innings_end(self, state: MatchState) -> Optional[str]:
        """Handle end of innings, return match result if game over"""
        if state.innings == 1:
            # First innings done, start second
            ...
            if team1_score > team2_score:
                ...
                return f"{state.team1} won by {margin} runs"
            elif team2_score > team1_score:
                wickets_left = 10 - state.wickets[1]
                return f"{state.team2} won by {wickets_left} wickets"
            else:
                return "Match tied"
            
    def get_match_phase(self, state: MatchState) -> str:
        """Determine current phase of innings"""
        overs = state.balls // 6
    ...
    def get_field_restrictions(self, state: MatchState) -> Dict[str, int]:
        """Get field restrictions for current phase"""
        phase = self.get_match_phase(state)
        
        if phase == "powerplay":
            return {
                "outside_circle": 2,  # Max 2 outside 30-yard circle
                "behind_square": 5    # Max 5 behind square
            }
        else:
            return {
                "outside_circle": 5,  # Max 5 outside circle
                "behind_square": 5    # Max 5 behind square
            }
    ...
    def simulate_ball(self, state:MatchState, model: 'PredictionModel') -> Tuple[Outcome, int]:
        ...
        outcome_probs = [probs.get(outcome.name.lower(), 0.0) for outcome in outcomes]
        ...
        # Sample outcome
        outcome = random.choices(outcomes, weights=outcome_probs)[0]
        ...
        # Handle over end if needed
        if state.balls % 6 == 0 and state.balls > 0:
            self.handle_over_end(state)
...
class XGBoostModel(PredictionModel):
    def __init__(self, model_path: str, batter_encoder_path: str, bowler_encoder_path: str):
        ...
        self.class_to_outcome = {
            0: 'dot', 1: 'one', 2: 'two', 3: 'four',  # Skip 3
    ...
    def extract_features(self, state: MatchState) -> np.ndarray:
        ...
        #Calculate features needed for xgboost model
        inning_idx = state.innings
        score = int(state.runs[team_idx])
        wickets = int(state.wickets[team_idx])
        balls_bowled = state.balls

        # For batter/bowler encoding, we'll use placeholders for now
        # In real implementation, you'd map player names/IDs
    ...
    # Dummy model for testing without trained model
...
if __name__ == "__main__":
    model = DummyModel()  # or XGBoostModel(...)
