"""B12 gate analysis — B10 selector re-gated on bowler_wkts_1plus at fresh seed 44.

WRITTEN AND COMMITTED BEFORE EITHER EVAL RESULT EXISTED.

B10 (TABLED, seed 43) share-matched the usage-absent branch of
`EmpiricalBowlerSelector` to B9's as-of expected-balls share. Its pre-committed
`top_bowler` primary was flat, but the post-hoc guard scan found a CI-clean
improvement on `bowler_wkts_1plus` (-0.0049 [-0.0075,-0.0023]). Because that
signal was identified POST HOC on seed-43 draws, B12 re-runs the same arm pair
on FRESH Monte Carlo draws (seed 44) with `bowler_wkts_1plus` PRE-COMMITTED as
the primary.

Two prop_backtest detail JSONs, both fresh in-session at seed 44, n=261 x 100
...
ABSENT from `models/bowler_phase_usage.json`:
  ...
  blind  models/auto/b12/detail_blind_s44_n261.json
         current default path (flat alpha = k * league_share for unknowns)
  b10    models/auto/b12/detail_b10_s44_n261.json
         unknowns re-weighted to their B9 as-of expected-balls share
         (`--bowler-usage-path models/auto/b10/bowler_phase_usage_b10.json`)

PRE-COMMITTED GATE (per research/handoff/B12/plan.md):

  GATE 1 (primary): `bowler_wkts_1plus` paired dBrier (b10 - blind) improves
    CI-clean (95% CI entirely < 0) at seed 44.
  GATE 2 (guards): no CI-clean regression (CI lower bound > 0 on a Brier /
    MAE family) on `top_bowler`, `bowler_wkts_2plus`, `batter_runs_mae`,
    `team_first_over_mae`.

  Both -> LANDED; exactly one -> TABLED; none -> FAILED.
  This script PRINTS the mapping; the ORCHESTRATOR issues the verdict.

G5 bowler coverage is reported for BOTH arms as CONTEXT ONLY — it is a gate
component in B10, not in B12.

The D15 drift-check section from b10_gate_analysis.py is DROPPED: both arms
here are fresh in-session twins on the same code, so there is nothing to drift
against.

PAIRING NOTE (pre-committed, identical machinery to B10/D15/A8): paired
...
Run:
  uv run python scripts/auto/b12_gate_analysis.py
"""
...
def main():
    ...
    ap.add_argument("--blind",
                    default=str(REPO / "models/auto/b12/detail_blind_s44_n261.json"))
    ap.add_argument("--b10",
                    default=str(REPO / "models/auto/b12/detail_b10_s44_n261.json"))
    ap.add_argument("--b10-log",
                    default=str(REPO / "research/handoff/B12/raw/run_b10.log"))
    ...
    print(f"blind: {len(da)} matches | b10: {len(db)} matches  (seed 44)\n")

    print("=" * 96)
    print(f"GATE 1 (PRIMARY) — {GATE1_FAMILY} paired Brier (b10 - blind) must "
          "be CI-clean negative")
    print("=" * 96)
    g1 = report(da, db, [GATE1_FAMILY], "blind", "b10", show_positional=True)
    tb = g1.get(GATE1_FAMILY)
    gate1 = bool(tb and tb[2] < 0)
    print(f"\n  GATE 1: {'MET' if gate1 else 'NOT MET'}")
    ...
    print("\n" + "=" * 96)
    print(f"CONTEXT — G5 bowler coverage, both arms (NOT a B12 gate; B10 used "
          f"a >= {G5_REPORT_FLOOR} floor)")
    print("=" * 96)
    truth = real_bowlers(Path(args.test_dir))
    g5_coverage(da, truth, "blind")
    g5_coverage(db, truth, "b10")
    ...
    print("\n" + "=" * 96)
    print("CONTEXT — B9 sim-vs-usage top_bowler margin "
          "(seed-43 reference: +0.0028 blind -> +0.0026 b10)")
    print("=" * 96)
    if not args.skip_b9:
        try:
            b9 = load_b9()
    ...
    print("\n" + "=" * 96)
    print("CONTEXT — B10 relaxation triggers (from the Arm-B run log)")
    print("=" * 96)
