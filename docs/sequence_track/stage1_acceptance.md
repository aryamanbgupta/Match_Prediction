...
| 12.10 | DONE — user decision (2026-09-11, in conversation): the registered reading "no arm advances; C−B favourable recorded as stage 2 evidence" accepted. Stage committed as `4109fc1`. Ledger: `research/IDEAS.md` entry **SQ1** appended (PENDING → claim RUNNING 2026-09-11T13:47:15Z → verdict), `research/log_verdict.py verdict SQ1 FAILED --gate-json models/embeddings/seq_stage1/full/gate/C-A_50000.json --date 2026-09-11 --commit 4109fc1` with the result text; `results.tsv` row appended with `gate_sha256=038fdfac…`. Per Astra NOTE 10, `FAILED` is the ledger word for "advancement not established" (the gate vocabulary requires an exact match to the gate JSON's verdict); it does not mean inferiority; `B-A_50000.json` and `holm_50000.json` are named as supporting evidence in the row. Ledger files committed separately after this row |
