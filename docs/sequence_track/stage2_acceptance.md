...
| 8.3 (verified live, 2026-09-12 ~01:45) | **PASS, and this is the check the whole split rests on.** The same configuration's training signature is byte-identical across the two machines despite different hardware and a symlinked frame on one of them: `full` reads `f30f355fd05ef001b7cff753361eceb8ddb6a5499e2c5631e9d00c265929a204` on the laptop's seed 7 (trained from the worktree, where `frame_dir` and `models_embeddings` are symlinks, on an Apple M5 Pro) **and** on the mini's seed 13 (real directories, Apple M4); `fixed_decay` likewise matches the value the laptop logged. Machine provenance correctly distinguishes them (`machine: laptop` / `mini`, chip, `repo_is_worktree: true` with root `/Users/aryamangupta/CricML/MP_train_s7`, all four thread caps at 2, and the per-path symlink flags). Astra round 2's new MUST-FIX 1 was exactly that this would fail; the fix is confirmed in production, so the morning consolidation will admit both halves rather than refusing them. `COMPLETE.json` carries the signature, its components, and the four-artefact manifest |
| D7 path pre-checked on a **trained** checkpoint | `ownership_dependency_test.py` run on `full/seed_7` at 200 targets as a positive control against `recency` k=30's S(i): `max|d| = 1.839e+00`, `p99 = 1.646e+00`, 152 of 200 targets moving, `SEES RELAY`, exit 0. Larger than the one-epoch smoke's 0.752, as a trained model that actually uses history should be. The D10.12 recertification path is therefore known to work before the masked arms finish |
...
ablation reported +0.0053 on 0 of 5 seeds on a different frame. **Superseded wording, corrected 2026-09-12 after Astra gate 3 round 2:** an
earlier draft of this block called these "two independent five-seed
measurements". They are **not independent** — this is a reproduction within a
validation extension that shares seeds 7 and 13 with the two-seed screen, and a
different frame does not make two analyses independent. The interval excludes
zero adversely on both.
