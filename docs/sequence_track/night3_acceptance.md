...
| N7 | One commit on both machines: laptop worktree detached at the frozen commit; mini at the same commit; recorded here. | pending. |
| N8 | Smoke on each machine before launch: one 128-step run of `mlp_pool` completes and writes COMPLETE.json; wall seconds recorded in the runbook table. | pending. |
